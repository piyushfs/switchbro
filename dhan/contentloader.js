...
async function displayAccounts() {
    ...
    dhan_users.forEach((item, index) => {
        ...
        if (item['active']) {
            listhtml += `
                <span class="badge rounded-pill editAcc" role="button" id="${item['id']}" data-bs-toggle="modal" data-bs-target="#dhanEdit">
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    document.getElementById('saveDhanCreds').addEventListener('click', async function (event) {
        const dhanEdit = document.getElementById('dhanEdit');
        const accId = dhanEdit.querySelector(".currentuser").id
        var user = {}
        user[accId] = {}
        user[accId]['pin'] = dhanEdit.querySelector("#pin").value;
        await addStoredCreds(user)
    });

    document.getElementById('toggleDhanPwd').addEventListener('click', async function (event) {
        const dhanEdit = document.getElementById('dhanEdit');
        const pinfield = dhanEdit.querySelector("#pin")
        const togglepwd = dhanEdit.querySelector("#toggleDhanPwd")
    ...
    document.getElementById('dhanEdit').addEventListener('show.bs.modal', async function (event) {
        const triggerElement = event.relatedTarget;
        const accId = triggerElement.closest('.switchAcc').id;
        const dhanEdit = document.getElementById('dhanEdit');
        dhanEdit.querySelector("#toggleDhanPwd").innerHTML = SEEPWD
        dhanEdit.querySelector(".currentuser").id = accId
        dhanEdit.querySelector("#pin").type = "password"

        const dhan_users = await getStoredCreds()
        console.log(dhan_users, 'users')
        if (accId in dhan_users) {
            const item = dhan_users[accId]
            const pin = item['pin']
            if (pin) {
                dhanEdit.querySelector('#pin').value = pin;
            }
        }
    });
});
