...
async function switchUser(user, broker) {
    if (broker == "FYERS") {
...
async function addAccount(broker) {
    if (broker === "FYERS") {
...
async function reloadOrOpenTab(broker) {
    return new Promise((resolve) => {
        chrome.tabs.query({}, function (tabs) {
            ...
            if (broker == "FYERS") {
                for (const item of tabs) {
                    if (item['url'].includes('trade.fyers.in')) {
                        tabId = item.id;
...
function createDocumentCookieString(cookies) {
    return cookies.map(cookie => {
        ...
        if (cookie.expirationDate) {
            cookieString += `; expires=${new Date(cookie.expirationDate * 1000).toUTCString()}`;
        }
...
function setDocumentCookie(string) {
